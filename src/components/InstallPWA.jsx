...
function InstallPWA() {
  ...
  // Jangan pernah jalankan / munculkan pop-up install di halaman Login agar keyboard HP tidak terganggu
  const isLoginPage =
  ...
  useEffect(() => {
    // Jika sedang dibuka dalam mode aplikasi PWA terinstal, jangan tampilkan prompt
    const isStandalone =
    ...
    const handleBeforeInstallPrompt = (e) => {
      ...
      // Hanya tampilkan jika belum ditutup dan bukan di halaman login
      if (!hasDismissed && !isLoginPage) {
        setShowModal(true);
      }
    };
  ...
  const handleInstallClick = async () => {
    ...
    if (outcome === "accepted") {
      console.log("PWA Berhasil Terinstall");
    }
  ...
  // Jangan render apa pun jika di halaman login atau state showModal false
  if (!showModal || isLoginPage) return null;
